      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-6 lg:p-8 rounded-[24px] lg:rounded-[32px] border border-slate-200 shadow-sm min-w-0">
          <div className="flex items-center justify-between mb-6 lg:mb-8">
            ...
            <button onClick={onViewInventory} className="px-3 py-1.5 bg-slate-50 hover:bg-slate-100 rounded-lg text-slate-600 text-[10px] font-bold transition-all border border-slate-200">
              Manage
            </button>
          </div>
        ...
        <div className="bg-white p-6 lg:p-8 rounded-[24px] lg:rounded-[32px] border border-slate-200 shadow-sm flex flex-col min-w-0">
          ...
          <div className="flex-1 h-48 lg:h-64 w-full min-w-0">
            <ResponsiveContainer width="100%" height="100%" minWidth={0}>
              <PieChart>
                <Pie
                  data={modelStats.length > 0 ? modelStats : [{name: 'Empty', value: 1}]}
                  cx="50%"
                  cy="50%"
                  innerRadius={50}
                  outerRadius={70}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 lg:gap-4">
          ...
          {notifications.length === 0 && (
            <div className="col-span-full py-10 text-center">
              <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Registry Quiet</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="bg-white p-5 lg:p-8 rounded-[20px] lg:rounded-[32px] border border-slate-200 shadow-sm active:scale-95 transition-all">
      <div className="flex items-center justify-between mb-4 lg:mb-6">
        <div className={`p-2 lg:p-4 rounded-lg lg:rounded-2xl ${colorMap[color]}`}>
          {icon}
        </div>
        <span className="hidden lg:inline text-[9px] font-black px-2 py-1 bg-slate-50 text-slate-400 rounded-full uppercase">
          {trend}
        </span>
      </div>
      <div>
