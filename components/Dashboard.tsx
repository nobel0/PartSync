...
import { Part, Notification, AppConfig } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ parts, notifications, config, onViewInventory }) => {
  ...
  const modelStats = useMemo(() => {
    ...
    parts.forEach(p => { if (p.carModel) statsMap[p.carModel] = (statsMap[p.carModel] || 0) + 1; });
  ...
  return (
    <div className="space-y-6 lg:space-y-8 pb-12">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6">
        ...
        <StatCard label="Total Vol" value={totalStock.toLocaleString()} icon={<ICONS.Dashboard />} trend="Units" color="indigo" />
        <StatCard label="Activity" value={notifications.length.toString()} icon={<ICONS.Suppliers />} trend="Events" color="cyan" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="lg:col-span-2 bg-white p-6 lg:p-8 rounded-[32px] border border-slate-200 shadow-sm min-w-0">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h3 className="text-lg lg:text-xl font-black text-slate-900">{config.labels.dashboardHeadline}</h3>
              <p className="text-[10px] lg:text-sm text-slate-500 uppercase font-bold tracking-widest">{config.labels.dashboardSubline}</p>
            </div>
            <button onClick={onViewInventory} className="px-3 py-1.5 bg-slate-50 border rounded-lg text-slate-600 text-[10px] font-bold">Manage</button>
          </div>
          <div className="h-64 lg:h-80 w-full min-w-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stockChartData} margin={{ top: 10, right: 0, left: -30, bottom: 0 }}>
                ...
                <Bar dataKey="stock" radius={[4, 4, 0, 0]} fill="#2563eb" barSize={24} />
                <Bar dataKey="target" radius={[4, 4, 0, 0]} fill="#e2e8f0" barSize={24} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>

        <div className="bg-white p-6 lg:p-8 rounded-[32px] border border-slate-200 shadow-sm flex flex-col min-w-0">
          <div className="mb-8">
          ...
          <div className="flex-1 h-48 lg:h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
...
const StatCard: React.FC<{ label: string; value: string; icon: React.ReactNode; trend: string; color: string }> = ({ label, value, icon, trend, color }) => {
  const colors: Record<string, string> = { blue: 'text-blue-600 bg-blue-50', red: 'text-red-600 bg-red-50', indigo: 'text-indigo-600 bg-indigo-50', cyan: 'text-cyan-600 bg-cyan-50', green: 'text-emerald-600 bg-emerald-50' };
  return (
    <div className="bg-white p-5 lg:p-8 rounded-[32px] border border-slate-200 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <div className={`p-4 rounded-2xl ${colors[color]}`}>{icon}</div>
        <span className="text-[9px] font-black px-2 py-1 bg-slate-50 text-slate-400 rounded-full uppercase">{trend}</span>
      </div>
