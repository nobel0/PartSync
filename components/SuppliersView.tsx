          ...
          return (
            <div key={s as string} className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm flex flex-col">
              <div className="flex items-center justify-between mb-6">
                <div className="p-3 bg-slate-900 text-white rounded-2xl"><ICONS.Suppliers /></div>
                {stats.critical > 0 && <span className="bg-red-100 text-red-600 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest animate-pulse">Needs Restock</span>}
              </div>
              ...
              <div className="mt-4 space-y-3 text-xs font-bold text-slate-500">
                <div className="flex justify-between"><span>SKUs</span><span className="text-slate-900">{stats.total}</span></div>
                <div className="flex justify-between"><span>Critical</span><span className={stats.critical > 0 ? "text-red-600" : "text-emerald-600"}>{stats.critical}</span></div>
              </div>
              <button className="mt-8 w-full py-4 bg-slate-50 text-slate-600 rounded-2xl text-[10px] font-black border border-slate-100 uppercase tracking-widest">Portal</button>
            </div>
          );
        })}
      </div>
