...
const AdminPanel: React.FC<AdminPanelProps> = ({ config, onSaveConfig, onDataRefresh }) => {
  ...
  const handleRemoveUser = (id: string) => {
    if (id === 'admin_01') return alert("Access Denied.");
      ...
      <div className="bg-white rounded-[32px] border border-slate-200 shadow-xl p-8 lg:p-12 min-h-[500px]">
        ...
        {activeTab === 'REGISTRY' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
            ...
            <div className="space-y-6">
              <h4 className="text-lg font-black text-slate-900 uppercase">Areas</h4>
              <div className="flex gap-2">
                  ...
                  <div>
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-1">
                         <button onClick={() => moveColumn(idx, 'UP')} className="p-1 hover:bg-slate-100 rounded text-slate-400">▲</button>
                         <button onClick={() => moveColumn(idx, 'DOWN')} className="p-1 hover:bg-slate-100 rounded text-slate-400">▼</button>
                      </div>
                    </div>
        ...
        {activeTab === 'PERSONNEL' && (
          <div className="space-y-10">
            <div className="bg-slate-900 p-8 rounded-[32px] text-white space-y-6 shadow-2xl">
              <h4 className="text-lg font-black uppercase tracking-tight">Enroll Personnel</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <select className="px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-xs font-bold text-white" value={newUser.assignedLine} onChange={e => setNewUser({ ...newUser, assignedLine: e.target.value })}>
                  <option value="ALL" className="text-slate-900">Global</option>
                  {formData.manufacturingShops.map(s => <option key={s} value={s} className="text-slate-900">{s}</option>)}
                </select>
                <button onClick={handleAddUser} className="bg-blue-600 hover:bg-blue-500 py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg">Register</button>
              </div>
            </div>
                ...
                <div key={u.id} className="p-6 rounded-[24px] border border-slate-100 bg-white shadow-sm flex flex-col justify-between hover:shadow-md transition-all">
                  <div>
                    <div className="flex items-center justify-between">
                      ...
                      {u.id !== 'admin_01' && <button onClick={() => handleRemoveUser(u.id)} className="text-slate-300 hover:text-red-500">✕</button>}
                    </div>
        ...
        {activeTab === 'CLOUD' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div className="space-y-6">
              ...
              <button onClick={() => storageService.setDBCredentials(dbCreds)} className="w-full py-4 bg-slate-900 text-white rounded-[20px] font-black text-xs shadow-lg">Link Cloud</button>
            </div>
          </div>
        )}
           ...
           <div className="space-y-12">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div className="bg-emerald-900 p-10 rounded-[40px] text-white space-y-6">
                    <h4 className="text-xl font-black">Excel</h4>
                    <button onClick={handleExportExcel} className="w-full py-4 bg-white text-emerald-900 rounded-2xl font-black text-xs uppercase">Export Registry</button>
                 </div>
              </div>
           </div>
        )}
